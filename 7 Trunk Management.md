...
VoIP服务提供商托管电话线，取代传统的电信线路，提供中继服务。供应商可以在一个或多个城市或国家分配本地号码，并将这些号码转给你。在大多数情况下，他们还支持号码移植。

服务供应商能够提供更好的通话费率，因为他们可能有一个国际网络，或者已经协商好更好的费率。因此，使用VoIP供应商可以降低通话成本。

PortSIP PBX支持这些中继线类型。

 + 基于注册的中继：这种中继需要PBX通过认证ID和密码来注册中继。"系统管理员 "可以配置一个 "基于注册 "的中继，并将其分配给租户，同时为每个租户分配一个DID号码池。租户共享该中继线，但有不同的DID号码池。"租户管理员 "也可以配置一个 "基于注册的 "中继，但是，这个中继不能与其他租户共享，其主机名和认证ID不能与其他中继相同。

 + 接受注册中继。该中继线 运营商使用预定义的认证 ID 和密码注册到 PBX。"系统管理员 "可以配置 "接受注册 "中继，并将其分配给租户，以及为每个租户分配一个DID池。"租户管理员 "也可以配置一个 "接受注册 "中继，但是，这个中继不能与其他租户共享，其主机名和认证ID不能与其他中继相同。

 + 基于IP的中继线。基于IP的中继线一般不要求PBX在中继线上注册。PBX的IP地址需要在中继侧进行配置，这样它就知道打到你的号码的电话应该被转到哪里。基于IP的 "中继线只能由 "系统管理员 "来配置，而且基于IP的中继线只能添加一次。如果 "系统管理员 "希望允许一个以上的租户使用该中继，"系统管理员 "可以将该中继分配给租户，并为每个租户分配一个DID池。

 + 微软团队。在PortSIP PBX中设置Microsoft Teams Direct Routing作为一个中继线。这个中继线只能由 "租户管理员 "配置，不能与其他租户共享。

租户管理员 "有能力查看 "系统管理员 "分配给他的所有中继，并在其基础上创建入站/出站规则，但他不能改变这些中继。

## 7.1 配置 Trunk

首先，你需要有一个VoIP服务提供商的账户。PortSIP PBX支持大多数流行的基于SIP的VoIP服务提供商。
...
由于 PortSIP PBX 是一个多租户的 PBX，如果多个租户在 PBX 中从同一中继供应商处建立了中继，并在呼入规则中设置了相同的 DID 号码，当呼叫到达 PBX 时，PBX 不知道应该将呼叫转接到哪个租户；而当一个租户的分机拨打中继时，设置的呼出号码是属于另一个租户的，就会造成问题。

为了解决上述问题，PortSIP PBX引入了 DID号码范围 的概念。

一旦一个租户被 "系统管理员 "分配到中继线，"系统管理员 "必须为该租户设置一个 DID号码范围 ，该 DID号码范围 不能与其他租户的 DID号码范围 重叠。当租户基于这个分配的中继线创建入站规则时，租户只能使用 DID号码范围 中的DID号码。

如果 "租户管理员 "为自己添加中继，必须为该中继指定一个 DID号码范围 。在添加该中继线后，为该租户创建呼入规则时，使用的DID号码必须在这个DID Pool范围内。 DID号码范围 不能与同一中继线提供者重叠。

例如，租户A添加了中继供应商XYZ的中继，并将 DID号码范围 设置为1000-2000；租户B也添加了同一中继供应商XYZ的中继，并将DID Pool设置为2000-3000；这将失败，因为同一中继的 DID号码范围 是重叠的；当2000号的呼叫进入时，PBX不知道应该将呼叫转到哪个租户。

DID Pool 允许一个单一的号码或一个号码范围，如下所示。
...
```
DID号码和 DID号码范围 不能以 "+"、"0 "或 "00 "开头；如果您的DID号码或 DID号码范围  以 "+"、"0 "或 "00 "开头，请在输入前删除。
```

### 系统管理员添加Trunk

 1.选择 "Call Manager > Trunks "菜单，并点击箭头按钮，选择你需要添加的中继线类型。

 2.为该中继线输入一个友好的名称，并在 "主机域或IP"、"端口"、"出站代理服务器 "和 "出站代理服务器端口 "字段中填写你从中继线服务提供商那里得到的详细信息。

 3.传输。PBX 与中继线通信所使用的传输方式，你应该咨询你的中继线服务商，选择合适的传输方式，目前支持 UDP、TCP 和 TLS。在添加中继线之前，必须在 PBX 中添加该传输。例如，如果你的供应商要求使用TCP，你应该在PBX中添加TCP传输，请参考传输管理部分。

 4.中继线的相关 IP。对于某些中继线供应商来说，它可能会从多个 IP 向 PBX 发送 INVITE 消息，而不是只从 "主机域或 IP "发送。你需要点击 "添加 "按钮，在这里输入每个IP来添加关联IP。

如果干线类型是 "基于注册"，点击 "下一步 "按钮，填写用户名/认证名称、密码和注册时间，作为你从干线供应商那里得到的账户细节。
...
6.发送 OPTIONS 消息的时间间隔（秒）：消息的发送频率，以及何时认为目的地不可用。默认是360秒。

点击 "下一步 "按钮，设置更多参数。

由于该中继线是由 "系统管理员 "添加的，"系统管理员 "需要选择一个或多个租户，允许他们访问该中继线。

一旦一个租户被分配了中继，必须为该租户设置DID池，如果为多个租户分配了中继，DID池号码不能重叠。

当租户根据这个分配的中继线创建入站规则时，租户只能使用DID池中的DID号码。
