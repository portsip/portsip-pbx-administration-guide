...
5. 默认的传输方式。
 + TCP端口5069：用于与PBX通信
 + TLS端口5067：用于与Microsoft Teams通信
 + WSS端口5065：用于提供WebRTC服务
 + UDP端口5066：用于提供正常的SIP服务
...
+ 添加 "**TCP** "传输，用于SBC与PBX的通信，请参考下面的截图，并在 "**网卡接口** "中选择 "**SBC内网IP**"。
...
+ 添加 "**TCP** "传输，让SBC与PBX通信，请参考下面的截图，在 "**网卡接口** "中选择 "**SBC内网IP**"。
